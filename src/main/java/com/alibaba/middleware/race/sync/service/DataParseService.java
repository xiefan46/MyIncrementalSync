...
public class DataParseService {
    ...
    public DataParseService(ConcurrentLinkedQueue<ParseTask> input, DataReplayService inRangeReplayService, DataReplayService outRangeReplayService) {
        this.input = input;
    ...
    public void start() {
        ...
        final Parser[] parsers = new Parser[Config.PARSER_NUM];
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    for (int i = 0; i < Config.INRANGE_REPLAYER_COUNT; ++i) replayServices[0].addTask(i, ReplayTask.END_TASK);
                    for (int i = 0; i < Config.OUTRANGE_REPLAYER_COUNT; ++i) replayServices[1].addTask(i, ReplayTask.END_TASK);
                    long end = System.currentTimeMillis();
                    ParseMetrics parseMetrics = new ParseMetrics();
                    for (int i = 0; i < parsers.length; ++i)
                        parseMetrics.merge(parsers[i].getParseMetrics());
                    stat.info("parse finish, cost {} ms, {}", end - start, parseMetrics.desc());
    ...
    class Parser implements Runnable {
        ...
        private void insert() {
            ...
            ByteBuffer partition = getBufferWithRemaining(flag, 1 + 8 + columnCount * 8, partitionId);
            partition.put(OPCode.INSERT);
        ...
        private void normalUpdate(long pk) {
            int flag = getFlag(pk);
            int partitionId = partitioners[flag].getPartitionId(pk);
            ByteBuffer partition = getBufferWithRemaining(flag, 1 + 8 + columnCount * (1 + 8), partitionId);
            partition.put(OPCode.UPDATE);
            partition.putLong(pk);
        ...
        private void updatePk(long oldPk, long newPk, long epoch) {
            ...
            partition = getBufferWithRemaining(flag, 1 + 8 * 3 + 9 * columnCount + 1, partitionId);
            partition.put(OPCode.UPDATE_PK)
                    .putLong(oldPk).putLong(newPk).putLong(waitId);
            putUpdateColumn(partition);
        }
        ...
        private void delete() {
            ...
            partition.put(OPCode.DELETE).putLong(pk);
...
//        private ByteBuffer localBuffer = ByteBuffer.allocate(Config.READ_BUFFER_SIZE);
        ...
        @Override
        public void run() {
            pools[0] = new BufferPool(Config.INRANGE_PARTITION_BUFFER_COUNT, Config.INRANGE_PARTITION_BUFFER_SIZE);
            pools[1] = new BufferPool(Config.OUTRANGE_PARTITION_BUFFER_COUNT, Config.OUTRANGE_PARTITION_BUFFER_SIZE);
